...
public class MaschineEnableStatusFilterRule1 implements IFilterRule<MaschineVO> {
	...
	public MaschineVO[] process(MaschineVO[] vos) {
		...
		if((dvos==null) || (dvos.length==0)){
			ExceptionUtils.wrappBusinessException("没有满足启用条件的记录！");
		}
	...
	private MaschineVO[] filterVOByVOState(MaschineVO[] vos) {
		...
		for(MaschineVO vo:vos){
			if(("0".equals(vo.getMstatus())) || ("2".equals(vo.getMstatus()))){
				list.add(vo);
			}
		}
