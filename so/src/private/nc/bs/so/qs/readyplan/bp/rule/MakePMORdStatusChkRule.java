...
public class MakePMORdStatusChkRule implements IRule<RdPorductDetailVO>{

	public MakePMORdStatusChkRule(){
	...
	public void process(RdPorductDetailVO[] vos) {
		...
		for (int i = 0; i < vos.length; i++) {
			...
			if(vos[i].getBillstatus()==2){
				failures.add(new ValidationFailure("生产日期：【"+vos[i].getProddate()+"】,生产数量：【"+vos[i].getProdnum()+"】，已经已经进入待生成列表，不能继续！"));
			}
